...
「**ラボ 02: Azure Repos での Git によるバージョン管理**」を完了しており、「**演習 0: ラボの前提条件の構成**」および「**演習 1: 既存のリポジトリを複製する**」の手順を既に完了している場合は、「**演習 2: Azure DevOps からブランチを管理する**」から始めてください。
...
1.  「**新しいプロジェクトの作成**」ページの 「**新しいプロジェクト名**」テキストボックスに、「**Version Controlling with Git in Azure Repos**」と入力し、「**組織の選択**」ドロップダウン リストで、Azure DevOps 組織を選択して、「**テンプレートの選択**」をクリックします。
...
1.  前の演習で生成した **Version Controlling with Git in Azure Repos** プロジェクトに移動します。 
...
1.  「**ブランチ**」 ペインを開き、右上の「**New branch**」 をクリックします。

1.  「**ブランチの作成**」 パネルの 「**名前**」 テキストボックスに「**release**」と入力し、「**Based on**」 ドロップダウン リストに **master**が表示されていることを確認し「**Create**」 をクリックします。
...
1.  Visual Studio Code ウィンドウの左下隅で、**master** エントリをクリックします。
...
1.  Azure DevOps ポータルの 「**ブランチ**」 ペインタブを表示している Web ブラウザーに切り替えます。 

1.  「**ブランチ**」 ペインの 「**Mine**」 タブで、**Release** ブランチ エントリにマウス ポインターを合わせると、右側に省略記号が表示されます。

1.  省略記号をクリックし、ポップアップ メニューで 「**ブランチの削除**」を選択し、確認を求められたら 「**削除**」をクリックします。

1.  「**ブランチ**」 ペインで「**All**」 タブを選択します。 

1.  「**ブランチ**」 ペインの 「**All**」 タブで右側の 「**ブランチ名の検索**」 テキスト ボックスに、「**release**」と入力します。 

1.  新しく削除されたブランチを表すエントリを含む 「**削除されたブランチ**」 セクションを確認します。 

1.  「**削除されたブランチ**」 セクションで、マウス ポインターを**リリース** ブランチ エントリに合わせると、右側に省略記号が表示されます。

1.  ポップアップ メニューで省略記号をクリックし、「**Restore branch**」 を選択します。
...
1.  Azure DevOps ポータルの 「**ブランチ**」 ペインを表示している Web ブラウザーに切り替えます。 

1.  「**ブランチ**」 ペインの 「**Mine**」 タブで、**マスター** ブランチ エントリにマウス ポインターを合わせると、右側に省略記号が表示されます。

1.  省略記号をクリックし、ポップアップ メニューで 「**ロック**」 を選択します。

1. master ブランチの名前の横に鍵マークがついたことを確認してください。
...
1.  Azure DevOps ポータルの垂直ナビゲーション ペインの 「**リポジトリ**」 セクションで、「**Tags**」 を選択します。

1.  「**Tags**」 ペインで、「**New tag**」 をクリックします。

1.  「**タグの作成**」 パネルの「**名前**」 テキストボックスに「**v1.1.0-beta**」と入力し、「**Based on**」 ドロップダウン リストで**master** エントリを選択したままにし、「**説明**」 テキスト ボックスに「**ベータリリース v1.1.0**」と入力して 「**作成**」 をクリックします。
...
1.  Azure DevOps ポータルを表示している Web ブラウザーの垂直ナビゲーション ペインで、プロジェクト名のすぐ右側にある「＋」記号をクリックし、「**新しいリポジトリ**」 をクリックします。
...
1.  「**リポジトリの作成**」 ペインの 「**リポジトリの種類**」 で、デフォルトの **Git** エントリをそのままにし、「**リポジトリ名**」 テキスト ボックスに「**新しいリポジトリ**」と入力し、他の設定をデフォルト値のままにして、「**作成**」 をクリックします。 
    ...
    > **注**: この時点で、リポジトリは使用可能です。これで、Visual Studio Code またはその他の git互換ツールを使用して、複製することができます。
...
1.  Azure DevOps ポータルを表示している Web ブラウザーで、垂直ナビゲーション ペインの下部にある 「**Project settings**」 をクリックします。

1.  「**プロジェクト設定**」 垂直ナビゲーション ペインで、「**Repos**」 セクションまで下にスクロールし、「**Repositories**」 をクリックします。
